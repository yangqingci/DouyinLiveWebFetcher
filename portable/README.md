...
采集器当前客户端编码是 `douyin_forwarder`，当前版本是 `1.1.0`。登录平台成功后，采集器会自动调用平台的 `/api/client/version/check` 检查是否有新版本。

如果平台发布了普通更新，窗口会提示新版本，可以点击“下载更新”打开下载地址；如果平台发布了强制更新，采集器会禁用“开始采集”，必须下载新版后再继续使用。
